  ...
  return (
    <>
      ...
      <div className="min-h-screen">
        {/* Hero */}
        <div className="bg-gradient-to-br from-primary-950 via-primary-900 to-slate-900 text-white pt-28 pb-16 md:pt-32 md:pb-24 px-4 relative overflow-hidden">
          ...
          <div className="max-w-4xl mx-auto text-center relative z-10">
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: "easeOut" }}>
              <div className="flex justify-center mb-5">
                <span className="px-4 py-2 rounded-full border border-cyan-400/30 bg-cyan-500/10 backdrop-blur-md text-cyan-300 font-semibold tracking-wide shadow-[0_0_20px_rgba(34,211,238,0.2)] text-sm">
                  Our Story
                </span>
              </div>

              <h1 className="font-display text-4xl sm:text-5xl md:text-7xl font-bold mb-6 md:mb-8 leading-tight tracking-tight">
              ...
              <p className="text-blue-100/90 text-lg md:text-xl lg:text-2xl leading-relaxed max-w-2xl mx-auto font-light">
        ...
        {/* Stats */}
        <div className="bg-white dark:bg-dark-bg border-b border-gray-100 dark:border-gray-800 py-8 md:py-10 px-4">
          <div className="max-w-5xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8 text-center">
            {[
              ...
              <div key={stat.label}>
                <p className="font-display text-3xl md:text-4xl font-bold text-primary-800 dark:text-primary-300">{stat.value}</p>
                <p className="text-gray-500 dark:text-gray-400 mt-1 text-sm md:text-base">{stat.label}</p>
              </div>
            ))}
          </div>
        </div>
            ...
            <div className="grid lg:grid-cols-2 gap-10 lg:gap-12 items-center">
              <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
                ...
                <div className="flex flex-col sm:flex-row gap-3 mt-8">
                  <Link to="/shop" className="btn-primary justify-center">Explore Collection <ArrowRight className="w-4 h-4" /></Link>
                  <Link to="/custom-order" className="btn-secondary justify-center">Custom Order</Link>
                </div>
              </motion.div>

              <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="relative mt-8 sm:mt-0">
                ...
                <div className="absolute -bottom-4 -left-3 sm:-bottom-6 sm:-left-6 card p-3 sm:p-4 shadow-xl z-10">
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-gold-100 dark:bg-gold-900/30 flex items-center justify-center flex-shrink-0">
                      <Award className="w-4 h-4 sm:w-5 sm:h-5 text-gold-600 dark:text-gold-400" />
                    </div>
                    <div>
                      <p className="font-bold text-xs sm:text-sm text-gray-900 dark:text-white">Best Furniture Store</p>
        ...
        {/* Values */}
        <section className="py-14 md:py-20 px-4 bg-gray-50 dark:bg-gray-900/30">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
              {values.map((value, i) => (
                <motion.div key={value.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="card p-5 md:p-6 text-center hover:-translate-y-1">
        ...
        <section className="py-14 md:py-20 px-4">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
              {team.map((member, i) => (
                <motion.div key={member.name} initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="card p-5 md:p-6 text-center">
                  <span className="text-5xl mb-3 block">{member.emoji}</span>
                  <h3 className="font-bold text-gray-900 dark:text-white">{member.name}</h3>
                  <p className="text-primary-700 dark:text-primary-300 text-sm font-medium mt-1">{member.role}</p>
        ...
        {/* CTA */}
        <section className="py-14 md:py-20 px-4 bg-gradient-to-br from-primary-950 via-primary-900 to-slate-900 text-white">
          <div className="max-w-3xl mx-auto text-center">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
              <h2 className="font-display text-3xl md:text-4xl font-bold mb-4">Ready to Transform Your Space?</h2>
              <p className="text-blue-200/80 mb-8 text-base md:text-lg">Visit our showroom or browse online. Free island-wide delivery on orders above Rs. 50,000.</p>
              <div className="flex flex-col sm:flex-row gap-3 justify-center">
                <Link to="/shop" className="btn-primary justify-center">Shop Now <ArrowRight className="w-4 h-4" /></Link>
